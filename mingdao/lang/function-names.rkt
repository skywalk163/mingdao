...
;; 明道语言内置函数名统一管理
;; 所有硬编码函数名集中在此模块，供解析器和其他模块使用

(provide builtin-function-names)
...
(define builtin-function-names
  '("打印" "长度" "索引" "列表" "列表修改" "消息拼接" "生成" "捕获" "任意" "新建" "定义类" "异步" "等待"
    ...
    "报错" "是否相等" "不等于" "任一" "符号判断" "列表判断" "符号转字符串"
    "数大于" "数小于" "数大于等于" "数小于等于"
    "前置" "追加多个" "去掉首个"
    "字符串转数字" "字符串转符号" "列表包含" "取前几个" "去掉前几个"
    ...
    "解析函数调用" "解析定义" "解析参数列表"
    "解析如果" "解析对于" "解析遍历"
    "解析返回" "解析赋值" "解析满足循环" "解析程序" "解析"
    ;; 模块系统关键字
    "公开" "私有" "模块" "作为" "使用" "版本" "导出"))